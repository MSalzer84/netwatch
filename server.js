...
app.get('/download/install-hyperv-agent.bat', (req, res) => {
  ...
  const lines = [
    ...
    ')',
    '',
    ':: Hyper-V pruefen (Win10/11 und Windows Server)',
    'powershell -Command "try { $f=Get-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V -EA Stop; if($f.State -ne \'Enabled\'){exit 1} } catch { $f=Get-WindowsFeature -Name Hyper-V -EA SilentlyContinue; if(-not $f -or $f.InstallState -ne \'Installed\'){exit 1} }" >nul 2>&1',
    'if %errorLevel% neq 0 (',
